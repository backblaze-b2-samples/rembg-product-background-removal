...
- Client route-construction tests: `pnpm --filter @rembg-product-background-removal/web test`
- Full verify command: `pnpm lint && pnpm --filter @rembg-product-background-removal/web test && pnpm build && pnpm lint:api && pnpm test:api && pnpm check:structure`
